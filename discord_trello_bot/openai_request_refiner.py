...
class OpenAIRequestRefiner:
    ...
    def refine(
        ...
        *,
        ...
        payload = {
            ...
            "store": False,
            "safety_identifier": self._build_safety_identifier(command_message.author_id),
            "input": [
                {
                    "role": "developer",
                    "content": (
        ...
        if not output_text:
            raise ValueError("Resposta vazia da OpenAI ao refinar o card.")

        try:
            data = json.loads(output_text)
        except json.JSONDecodeError as exc:
            raise ValueError("A OpenAI nao retornou JSON valido para o card.") from exc
        ...
        if not title or not summary:
            raise ValueError("A OpenAI retornou card sem titulo ou resumo.")
    ...
    @staticmethod
    def _build_safety_identifier(author_id: str) -> str:
